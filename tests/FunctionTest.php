...
class FunctionTest extends TestCase
{
    ...
    {
        $s = \PhMessage\stream_for('foobaz');
        $this->assertEquals('foobaz', \PhMessage\copy_to_string($s));
        $s->seek(0);
        $this->assertEquals('foo', \PhMessage\copy_to_string($s, 3));
        $this->assertEquals('baz', \PhMessage\copy_to_string($s, 3));
        $this->assertEquals('', \PhMessage\copy_to_string($s));
    }

    public function testCopiesToStringStopWhenReadFails()
    {
        $s = \PhMessage\stream_for('foobaz');
        $s = FnStream::decorate($s, [
            'read' => function() { return ''; },
            'eof' => function() { return true; }
        ]);
        $result = \PhMessage\copy_to_string($s);
    ...
    {
        $s1 = \PhMessage\stream_for('foobaz');
        $s2 = \PhMessage\stream_for('');

        \PhMessage\copy_to_stream($s1, $s2);
        ...
        \PhMessage\copy_to_stream($s1, $s2, 3);
    ...
    public function testCopiesToStreamStopWhenWriteFails()
    {
        $s1 = \PhMessage\stream_for('foobaz');
        $s2 = \PhMessage\stream_for('');
    ...
    public function testStopsCopyToSteamWhenWriteFailsWithMaxLen()
    {
        ...
        $s2 = \PhMessage\stream_for('');
        $s2 = FnStream::decorate($s2, ['write' => function () { return 0; }]);
        \PhMessage\copy_to_stream($s1, $s2, 10);
        $this->assertEquals('', (string) $s2);
    }
    ...
    public function testReadsLines()
    {
        ...
        $this->assertEquals("foo\n", \PhMessage\readline($s));
        $this->assertEquals("baz\n", \PhMessage\readline($s));
        $this->assertEquals("bar", \PhMessage\readline($s));
    }

    public function testReadsLinesUpToMaxLength()
    {
        $s = \PhMessage\stream_for("12345\n");
        $this->assertEquals("123", \PhMessage\readline($s, 4));
        $this->assertEquals("45\n", \PhMessage\readline($s));
    }

    public function testReadsLineUntilFalseReturnedFromRead()
    {
        $s = $this->getMockBuilder('\PhMessage\Stream')
            ->setMethods(['read', 'eof'])
        ...
        $this->assertEquals("h", \PhMessage\readline($s));
    }

    public function testCalculatesHash()
    {
        $s = \PhMessage\stream_for('foobazbar');
        $this->assertEquals(md5('foobazbar'), \PhMessage\hash($s, 'md5'));
    }
    ...
    {
        $s = new NoSeekStream(\PhMessage\stream_for('foobazbar'));
        $s->read(2);
        \PhMessage\hash($s, 'md5');
    }
    ...
    {
        $r = \PhMessage\try_fopen(__FILE__, 'r');
    ...
    {
        \PhMessage\try_fopen('/path/to/does/not/exist', 'r');
    }
    ...
    {
        $result = \PhMessage\parse_query($input);
    ...
    {
        $str = 'foo%20=bar';
        $data = \PhMessage\parse_query($str, false);
    ...
    {
        $result = \PhMessage\parse_query($input, false);
        $this->assertSame($input, \PhMessage\build_query($result, false));
    }

    public function testEncodesWithRfc1738()
    {
        $str = \PhMessage\build_query(['foo bar' => 'baz+'], PHP_QUERY_RFC1738);
        $this->assertEquals('foo+bar=baz%2B', $str);
    }

    public function testEncodesWithRfc3986()
    {
        $str = \PhMessage\build_query(['foo bar' => 'baz+'], PHP_QUERY_RFC3986);
        $this->assertEquals('foo%20bar=baz%2B', $str);
    }

    public function testCanControlDecodingType()
    {
        $result = \PhMessage\parse_query('var=foo+bar', PHP_QUERY_RFC3986);
        $this->assertEquals('foo+bar', $result['var']);
        $result = \PhMessage\parse_query('var=foo+bar', PHP_QUERY_RFC1738);
        $this->assertEquals('foo bar', $result['var']);
    }
    ...
    public function testParsesRequestMessages()
    {
        ...
        $request = \PhMessage\parse_request($req);
    ...
    {
        \PhMessage\parse_request("HTTP/1.1 200 OK\r\n\r\n");
    }

    public function testParsesResponseMessages()
    {
        $res = "HTTP/1.0 200 OK\r\nFoo: Bar\r\nBaz: Bam\r\nBaz: Qux\r\n\r\nTest";
        $response = \PhMessage\parse_response($res);
    ...
    {
        \PhMessage\parse_response("GET / HTTP/1.1\r\n\r\n");
    }

    public function testDetermineMimetype()
    {
        $this->assertNull(\PhMessage\mimetype_from_extension('not-a-real-extension'));
        $this->assertEquals(
            'application/json',
            \PhMessage\mimetype_from_extension('json')
        );
        $this->assertEquals(
            'image/jpeg',
            \PhMessage\mimetype_from_filename('/tmp/images/IMG034821.JPEG')
        );
    }

    public function testCreatesUriForValue()
    {
        $this->assertInstanceOf('\PhMessage\Uri', \PhMessage\uri_for('/foo'));
        $this->assertInstanceOf(
            '\PhMessage\Uri',
            \PhMessage\uri_for(new Uri('/foo'))
        );
    }
    ...
    public function testCreatesWithFactory()
    {
        ...
        $this->assertInstanceOf('\PhMessage\Stream', $stream);
    ...
    {
        $s = \PhMessage\stream_for();
        $this->assertInstanceOf('\PhMessage\Stream', $s);
    }

    public function testFactoryCreatesFromNull()
    {
        $s = \PhMessage\stream_for(null);
        $this->assertInstanceOf('\PhMessage\Stream', $s);
    }
    ...
    public function testFactoryCreatesFromObjectWithToString()
    {
        ...
        $s = \PhMessage\stream_for($r);
        $this->assertInstanceOf('\PhMessage\Stream', $s);
        $this->assertEquals('bar', (string) $s);
    }

    public function testCreatePassesThrough()
    {
        $s = \PhMessage\stream_for('foo');
        $this->assertSame($s, \PhMessage\stream_for($s));
    }
    ...
    {
        \PhMessage\stream_for(new \stdClass());
    }

    public function testReturnsCustomMetadata()
    {
        $s = \PhMessage\stream_for('foo', ['metadata' => ['hwm' => 3]]);
    ...
    {
        $s = \PhMessage\stream_for('', ['size' => 10]);
    ...
    public function testCanCreateIteratorBasedStream()
    {
        ...
        $p = \PhMessage\stream_for($a);
        $this->assertInstanceOf('PhMessage\PumpStream', $p);
    ...
    {
        $request = new Request('PUT', 'http://foo.com/hi?123', [
            'Baz' => 'bar',
            'Qux' => 'ipsum'
        ], 'hello', '1.0');
        $this->assertEquals(
            "PUT /hi?123 HTTP/1.0\r\nHost: foo.com\r\nBaz: bar\r\nQux: ipsum\r\n\r\nhello",
            \PhMessage\str($request)
        );
    }
    ...
    {
        $this->assertEquals($result, \PhMessage\parse_header($header));
    }

    public function testParsesArrayHeaders()
    {
        $header = ['a, b', 'c', 'd, e'];
        $this->assertEquals(['a', 'b', 'c', 'd', 'e'], \PhMessage\normalize_header($header));
    }

    public function testRewindsBody()
    {
        $body = \PhMessage\stream_for('abc');
        $res = new Response(200, [], $body);
        \PhMessage\rewind_body($res);
    ...
    public function testCanModifyRequestWithUri()
    {
        ...
        $r2 = \PhMessage\modify_request($r1, [
    ...
    {
        $r1 = new Request('GET', 'http://foo.com:8000');
        $r2 = \PhMessage\modify_request($r1, [
    ...
    {
        $r1 = new Request('GET', 'http://foo.com', ['User-Agent' => 'foo']);
        $r2 = \PhMessage\modify_request($r1, ['set_headers' => ['User-agent' => 'bar']]);
    ...
    public function testReturnsAsIsWhenNoChanges()
    {
        $r1 = new Request('GET', 'http://foo.com');
        $r2 = \PhMessage\modify_request($r1, []);
        $this->assertTrue($r2 instanceof Request);

        $r1 = new ServerRequest('GET', 'http://foo.com');
        $r2 = \PhMessage\modify_request($r1, []);
    ...
    {
        $r1 = new Request('GET', 'http://foo.com');
        $r2 = \PhMessage\modify_request($r1, ['set_headers' => ['foo' => 'bar']]);
    ...
    {
        $r1 = new Request('GET', 'http://foo.com', ['foo' => 'bar']);
        $r2 = \PhMessage\modify_request($r1, ['remove_headers' => ['foo']]);
        $this->assertNotSame($r1, $r2);
    ...
    {
        $r1 = new Request('GET', 'http://foo.com');
        $r2 = \PhMessage\modify_request($r1, ['query' => 'foo=bar']);
        $this->assertNotSame($r1, $r2);
    ...
    {
        $r1 = new Request('GET', 'http://foo.com');
        $r2 = \PhMessage\modify_request($r1, ['remove_headers' => ['non-existent']]);
        $this->assertTrue($r2 instanceof Request);
