...
class PumpStreamTest extends TestCase
{
    ...
    {
        $p = \PhMessage\stream_for(function ($size) {
            return 'a';
        });
    ...
    {
        $called = [];
        $p = \PhMessage\stream_for(function ($size) use (&$called) {
            $called[] = $size;
            return 'abcdef';
        });
        $this->assertEquals('a', $p->read(1));
    ...
    {
        $p = \PhMessage\stream_for(function () { return 'a'; });
    ...
    {
        $p = \PhMessage\stream_for(function () {});
